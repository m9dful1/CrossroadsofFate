...
## 42. Interactive Map Discovery Audit Fixes
[Date of modification: 2026-07-11]
Eighth feature audit (interactive map: location discovery, unlocks, travel, activities UI). ActivityManager, the discovery filter, and the map UI audited clean; the gaps were stale-refresh paths and a missing content net.
### 42.1 Map Refresh on Discovery-Input Changes
`updateInteractiveMapLocations()` recomputes which locations the player can see, but two mutation paths never triggered it: story choices (`onChoiceSelected` grows `visitedLocations` and can grant items — e.g. picking up the holy key at the crossroads chapel makes the Sacred Temple discoverable) and quest completion (`handleQuestCompletion` grants reward items and explicitly unlocks locations). A player could earn a discovery and open a map that didn't show it until some unrelated action refreshed the list. Both paths now refresh, and `showMap()` also recomputes on open as a safety net for any future mutation path.
### 42.2 Location Content Test
